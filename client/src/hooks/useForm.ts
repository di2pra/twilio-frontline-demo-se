...
const useForm = (
  ...
  const validateState = useCallback(() => {
    ...
    const hasErrorInState = Object.keys(validationSchema).map(key => {
      ...
      if (isInputFieldRequired && (stateValue === null || stateValue === '') && !stateIsInvalid) {
      ...
      return (isInputFieldRequired && (stateValue === null || stateValue === '')) || stateIsInvalid;
    });
  ...
  const handleOnSubmit = useCallback(
    (e: React.FormEvent<HTMLFormElement>, callback: (state: FormSchema) => void) => {
      e.preventDefault();
  ...
  return { state, handleOnChange, handleOnSubmit, resetState};
}
